"use client";

import Link from "next/link";
import Image from "next/image";
import { FaGithub } from "react-icons/fa";
import { Project } from "@/types/types";

export const ProjectCardHigh = ({ project }: { project: Project }) => {
  const formatDate = (dateString: string) => {
    return new Intl.DateTimeFormat("en-US", {
      year: "numeric",
      month: "long",
      day: "numeric",
    }).format(new Date(dateString));
  };
  return (
    <div className="relative bg-transparent border-blue-500 border-2 text-white p-6 rounded-lg shadow-lg transition-transform transform hover:scale-105 hover:shadow-2xl hover:bg-transparent overflow-hidden">
      <div className="mb-6 rounded-lg overflow-hidden">
        <Image
          src={project.image[0]}
          alt={`Image Not available`}
          className="w-full h-56 object-cover rounded-lg transition-all duration-300 ease-in-out hover:scale-110"
          width={500}
          height={300}
        />
      </div>
      <h1 className="text-2xl font-semibold text-[#b9b9e3] mb-2">{project.name}</h1>

      <p className="text-white/40 text-sm mb-4">{project.description}</p>
      <div className="flex justify-between items-center  text-white">
        <Link className="flex items-center text-indigo-600 hover:text-indigo-800 transition-colors" href={project.github}>
          <FaGithub className="mr-2" />
          GitHub Repo
        </Link>
        <span className="text-sm">{formatDate(project.completionDate)}</span>
        <Link href={`/projects/${project._id}`} className="text-red-500 inline-block">
          Read More →
        </Link>
      </div>

      <div className="mt-4 text-sm font-bold text-gray-700 gap-2">
        Status:
        <span
          className={project.status === "Completed" ? "text-green-500 px-2" : "text-yellow-500 px-2"}
        >
          {project.status}
        </span>
      </div>

    </div>
  );
};
